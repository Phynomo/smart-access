...
npm run build      # production build → dist/smart-access-app/browser/
npm run watch      # incremental dev build with watch mode
npm test           # unit tests with Karma + Jasmine
```

Angular CLI shortcuts:
```bash
ng generate component <name>   # creates component with SCSS style by default
...
- `src/app/app.config.ts` → global providers (router, PrimeNG Aura theme, zone change detection)
- `src/app/app.routes.ts` → route definitions (currently empty, to be populated)
...
- Web dir points to `dist/smart-access-app/browser` — must run `npm run build` before syncing to native platforms.

**TypeScript:** Strict mode enabled with `noImplicitOverride`, `noImplicitReturns`, `strictTemplates`, and `typeCheckHostBindings`. All templates are type-checked at compile time.
